...
const LandingPage = ({ theme: { Color } }) => {
  ...
  const HandleQueries = useCallback(() => {
    if (tabItem.toLowerCase() === "recent") {
      Axios.get(`${process.env.NEXT_PUBLIC_APP_DOMAIN}/explore/recent-posts`)
        .then((res) => {
          setTab(res.data);
      ...
      Axios.get(`${process.env.NEXT_PUBLIC_APP_DOMAIN}/trending-posts`)
        .then((res) => {
          setTab(res.data);
      ...
      Axios.get(
        `${process.env.NEXT_PUBLIC_APP_DOMAIN}/explore/most-viewed-posts`
      )
        .then((res) => {
          setTab(res.data);
          setLoading(false);
