...
const FlexCenterBody = ({     error,     loading, FlexBodyArray, theme: {Color}, MessageBox,selectItem, singlepage, ProfileCommentBox }) => {
    ...
    const {user,sharedState} = useUser();
    const [messageBox, setMessageBox] = useState(false);
    const [pic, setPic] = useState();
    const [hasMore, setHasMore] = useState(false);
const postData = FlexBodyArray ? FlexBodyArray : sharedState?.channelPost
    // function fetchMore() {
    ...
    const LikePost = useCallback((postId) => {
        try {
            ...
            Axios.get(`${process.env.NEXT_PUBLIC_APP_DOMAIN}/like-post/${user?.id}/${postId}/agree`).then((res) => {
    ...
    const disLikePost = useCallback((postId) => {
        ...
        if (user === null) return router?.push("/auth/signin");
        Axios.get(
            `${process.env.NEXT_PUBLIC_APP_DOMAIN}/like-post/${user.id}/${postId}/disagree`
        )
            .then((res) => {
        ...
        <FlexCenterBodyStyles Color={Color}>
            {loading ? (
                ...
                //     }
                // >
              postData?.data?.map((item, keyMain) => (
